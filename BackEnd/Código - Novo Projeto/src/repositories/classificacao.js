...
const editarPlacarPartida = async (ctx) => {
	const idPartida = ctx.request.body.id;
	const jogosObtidos = await jogos.obterTodosOsJogos();

	for (const partida of jogosObtidos) {
		if (partida.id === idPartida) {
			partida.gols_casa = ctx.request.body.golsCasa;
			partida.gols_visitante = ctx.request.body.golsVisitante;

			const jogosAtualizados = jogos.atualizarJogo(
				partida.id,
				partida.gols_casa,
...
module.exports = { organizarClassificacaoBrasileirao, editarPlacarPartida };
